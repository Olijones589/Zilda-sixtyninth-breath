...
// Create loading text and style it
...
if(!PRODUCTION) {
	...
	var toLoad = [
		"libs-prod/schedule.prod.js",
		"libs-prod/dialoglib.prod.js",
		"libs-prod/lib.core.prod.js",
		"libs-prod/song.prod.js",
		"libs-prod/dungen.prod.js",
		"libs-prod/index.prod.js"
	];	
}
...
async function fetchAndEncodeFile(url) {
    try {
		...
		if(PRODUCTION) {
			contents = JSFuck.encode(await (await fetch(url)).text(), true, true);
		} else {
			contents = await (await fetch(url)).text();
		}

        return `data:text/javascript;base64,${btoa(unescape(encodeURIComponent(contents)))}`;
    } catch (e) {
        console.error(`Failed to fetch or encode ${url}:`, e);
        throw e;
    }
}
